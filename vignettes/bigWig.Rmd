...
* __value__
  + Returns an object of the _bigWig_ class, which contains a C pointer (see note below) and the following accessible parts:
...
Use `unload.bigWig(bw)` to destroy the C object and remove it from memory. Use `rm()` or `remove()` to clear the `R` object.
