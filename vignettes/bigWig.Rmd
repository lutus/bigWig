...
bw=load.bigWig('../inst/extdata/bp.bigWig')
```
The all of the attributes of the object can be accessed using `atrributes` and each individual can be accessed using `$`
...
## bwMap argument
All the `*.bpQuery.bigWig` functions can incorporate mappability information into the queries. The `strand` argument needs to be specified for non-`bed6` function, because mappability is dependent upon on the strand. The intervals than span unmappable positions return NA.   

```{r eval=TRUE}
bwMap = load.bwMap('../inst/extdata/bwMap.bigWig', read.len = 30,
               read.left.edge=FALSE)

bed6.region.bpQuery.bigWig(bwPlus, bwMinus, bed6,
                           bwMap=bwMap)
```
The `bwMap` arguments incorporates the `strand`, either from the _bed6_ data frame or the argument, with the `read.len` and `read.left.edge` attributes from the`bwMap` R object. 
...
Profile functions either calculate the quantile cutoff or confidence interval for data matrices. The `metaprofile.bigWig` function creates a class object that can be passed on to the matrix scaling or plotting functions.
...
`quantiles.metaprofile` invokes _R_'s `quantile` function for each column of the matrix; passing  _tss.matrix_, to `quantiles.metaprofile` we observe that the the majority of windows are zero, because the input _bigWig_ was randomly sampled, so it is a low coverage _bigWig_. The result of `quantiles.metaprofile` is a list of `quantile` values for the number and step size. 
...
The `subsampled.quantiles.metaprofile` function randomly subsamples the matrix, as determined by `fraction` and `n.samples`. Then the column means, `colMeans`, are calculated for each matrix , then `quantiles.metaprofile` calculates the quantiles for the matrix that results from concatenating the column mean values. Stated differently, a matrix is generated that contains `n.samples` rows and each row is a column mean from a subsample of the data, then `quantiles.metaprofile` is applied to the matrix.
...
composite.plus = metaprofile.bigWig(bedTSSwindow, plusPRO,
                 bw.minus = minusPRO, step = 50, name = "Signal",
                 matrix.op = zeroToOne.scale,
                 profile.op = bootstrapped.confinterval.metaprofile,
                 alpha = 0.05, n.samples = 1000)

composite.min = metaprofile.bigWig(bedTSSwindow, minusPRO,
                 bw.minus = plusPRO, step = 50, name = "Signal",
                 matrix.op = zeroToOne.scale,
                 profile.op = bootstrapped.confinterval.metaprofile,
                 alpha = 0.05, n.samples = 1000)

plot.metaprofile(composite.plus, minus.profile = composite.min, X0 = 275,
      draw.error = TRUE, col = c("red", "blue", "lightgrey", "lightgrey"),
      ylim = NULL, xlim = c(-225,225), xlab = "Distance (bp)",
      ylab = "PRO-seq signal")
