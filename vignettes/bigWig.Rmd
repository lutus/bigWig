...
A standard three column BED file is a tab delimited file that consists
...
window. These functions are strand specific. For a BED6 file,
...
Obviously,`sizes =
bed[,3] - bed[,2]` is much faster, but the function can be designed as 
...
defined by the corresponding column of a BED _data.frame_. The output
is a vector that is equal in length to the number of rows of the BED
_data.frame_. The vectors's index corresponds to the BED _data.frame_
row index.
...
plot(seq(-249, 250, by = 10),colSums(tss.matrix),
...
plot(seq(-249, 250, by = 10),colSums(tss.matrix.notStrand),
...
```{r eval=TRUE}
bigwig.map=load.bwMap('../inst/extdata/bwMap.bigWig', read.len = 30, read.left.edge=FALSE, threshold.fraction = 0.19)
...
#what base does each bwMap refer to?
bwMap.right=load.bwMap('../inst/extdata/bwMap.bigWig', read.len = 30, read.left.edge=FALSE)
bwMap.left=load.bwMap('../inst/extdata/bwMap.bigWig', read.len = 30, read.left.edge=TRUE)


region.bpQuery.bwMap(bwMap.left, "chr1", 134, 135, strand = "-", op = "sum")
region.bpQuery.bwMap(bwMap.left, "chr1", 105, 106, strand = "+", op = "sum")

region.bpQuery.bwMap(bwMap.right, "chr1", 105, 106, strand = "-", op = "sum")
region.bpQuery.bwMap(bwMap.right, "chr1", 134, 135, strand = "+", op = "sum")
```

The operation that in introduced with mappability function is `thresh`. It is reasonable to consider intervals as mappable as long as the average mappability of an interval exceeds a threshold. Here `thresh` refers to the average _unmappability_, since a value of 1 indicates the position is not mappable. Therefore, a threshold of 0.2 means that if 20% or more of the positions in the interval are not mappable, then the entire interval is considered to be unmappable.  
```{r eval=TRUE}
#illustrate threshold
bwMap=load.bwMap('../inst/extdata/bwMap.bigWig', read.len = 30, read.left.edge=FALSE, threshold.fraction = 0.2)
